...
class SigScan(Module):
    ...
    def __init__(self):
        ...
        self.signatures = [
            ...
            # Images
            (b'\xFF\xD8\xFF\xE0', 'JPEG Image', 'JPEG/JFIF image format', '.jpg/.jpeg'),
            (b'\xFF\xD8\xFF\xE1', 'JPEG Image (EXIF)', 'JPEG with EXIF data', '.jpg/.jpeg'),
            (b'\x89\x50\x4E\x47\x0D\x0A\x1A\x0A', 'PNG Image', 'Portable Network Graphics image', '.png'),
            (b'\x47\x49\x46\x38\x37\x61', 'GIF Image (87a)', 'GIF image version 87a', '.gif'),
            (b'\x47\x49\x46\x38\x39\x61', 'GIF Image (89a)', 'GIF image version 89a', '.gif'),
            ...
            (b'\x66\x74\x79\x70\x4D\x53\x4E\x56', 'MP4 Video', 'MP4 video file', '.mp4'),
            (b'\x66\x74\x79\x70\x69\x73\x6F\x6D', 'MP4 Container', 'ISO Base Media file/MP4 container', '.mp4/.m4v'),
            ...
            (b'\x1F\x8B\x08', 'GZip Archive', 'GZip compressed archive', '.gz'),
            ...
            (b'\x7B\x5C\x72\x74\x66\x31', 'RTF Document', 'Rich Text Format document', '.rtf'),
            (b'\x23\x21\x2F\x62\x69\x6E\x2F', 'Shell Script', 'Unix shell script (shebang)', '.sh'),
    ...
    def _find_signatures(self, data):
        ...
        This improved version only looks at the start of the file and specific offsets.
        
        Args:
            data (bytes): The file data to scan
            
        Returns:
            list: List of dictionaries containing signature information
        """
        results = []
        
        # Check for signatures at the start of the file (most common case)
        for sig, name, desc, ext in self.signatures:
            # Make sure we have enough data to check
            if len(data) >= len(sig):
                # Check if the signature appears at the start of the file
                if data[:len(sig)] == sig:
        ...
        # If no match at the beginning, check for common embedded signatures
        if not results:
            # Define common offsets where embedded files might start
            common_offsets = [0x200, 0x400, 0x800, 0x1000, 0x2000, 0x4000, 0x8000]
            
            # Check each offset for potential signatures
            ...
            # If still no matches, scan through the file looking for common container signatures
            # But limit to the first 100KB to prevent excessive scanning
            scan_size = min(len(data), 102400)  # 100KB
            target_sigs = [b'\x50\x4B\x03\x04', b'\xFF\xD8\xFF', b'\x52\x49\x46\x46']  # ZIP, JPEG, RIFF
            
            for i in range(scan_size):
                for sig in target_sigs:
                    if i + len(sig) <= len(data) and data[i:i+len(sig)] == sig:
                        # Found a target signature, look it up in our database
                        for full_sig, name, desc, ext in self.signatures:
                            if full_sig.startswith(sig):
                                hex_sig = ' '.join([f'{byte:02X}' for byte in full_sig])
                                result = {
                                    'offset': i,
                                    'name': name,
                                    'hex': hex_sig,
    ...
    def run(self):
        ...
        try:
            ...
            # Handle scan options
            if args.scan:
                self.do_scan(detailed=False)
            elif args.detailed:
                self.do_scan(detailed=True)
            else:
                # If no recognized args were provided, show usage
                self.usage()
                
        except Exception as e:
            # Catch any argument parsing errors and show usage
            self.log('error', f'Error parsing arguments: {str(e)}')
            self.usage()
    
    def do_scan(self, detailed=False):
        """
        Performs the signature scan operation.
        ...
        results = self._find_signatures(data)
    ...
    def usage(self):
        ...
        self.log('info', '  --scan      : Scan file for common file signatures (basic info)')
